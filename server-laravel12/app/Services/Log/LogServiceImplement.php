...
class LogServiceImplement  implements LogService
{
    ...
    public function all()
    {
        ...
        if ($perPage) {
            $logs = $this->loadRelationships($this->logRepository->filter($userName, $time))->orderByDesc('project_logs.updated_at')->paginate(is_numeric($perPage) ? $perPage : 10);
            return [
                'pagination' => $this->paginate($logs),
                'data' => LogResource::collection($logs->items())
            ];
        } else {
            $logs = $this->loadRelationships($this->logRepository->filter($userName, $time))->orderByDesc('project_logs.updated_at')->get();
            return [
                'data' => LogResource::collection($logs->items())
            ];
        }
