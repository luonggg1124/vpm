...
import { ITask, TaskStatus, taskStatusString } from "@/api/interfaces/ITask";
...
const UpdateTask = () => {
  ...
  const { data: projectData, isLoading: loadingProject } = useQueryConfig(
    [PATH_PROJECT.QUERY_KEY, PATH_PROJECT.QUERY_KEY + id + `update-task`],
    PATH_PROJECT.FIND.ROUTE + `/${id}`
  );
  const {
    data: taskData,
    isLoading: loadingTask,
    isSuccess: successTask,
  } = useQueryConfig(
    [PATH_TASK.QUERY_KEY, PATH_TASK.QUERY_KEY + id + taskId + `update-task`+`?include=designated_personnel,designating_personnel`],
    PATH_TASK.FIND.ROUTE(taskId)+`?include=designated_personnel,designating_personnel`
  );
  const handleBack = () => {
    navigate(-1);
  };
  const { state: requestUserId } = useLocalStorageState(USER_INFO.ID, 1);
  const { data: usersData, isSuccess: successTaskUser } = useQueryConfig(
    [
      PATH_USER.ALL.QUERY_KEY +
        `?${PATH_USER.ALL.Filter([requestUserId])}-update-tasks`,
    ],
    PATH_USER.ALL.ROUTE + `?${PATH_USER.ALL.Filter([requestUserId])}`
  );
  const { loading, updateTask } = useTask();
  const task: ITask = (taskData as any)?.data?.data || null;
  ...
  useEffect(() => {
    if (task && successTask && successTaskUser) {
      setTimeout(() => {
        form.reset({
          name: task?.name,
          uuid: task?.uuid,
          ...
          project_id: id,
          designated_personnel_id: String(task?.designated_personnel?.id),
          status: task.status !== TaskStatus.Done && new Date(task?.ended_at ? task.ended_at : "").getTime() < new Date().getTime() ? TaskStatus.Overdue : String(task?.status),
          ended_at: formatDate(
            new Date(task?.ended_at ? task?.ended_at : "").getTime()
          ),
          description: task?.description,
        });
      }, 0);
    }
  }, [task, successTask, successTaskUser]);
  ...
  const onSubmit = async (value: any) => {
    ...
    const error = await updateTask(taskId,value);
      ...
      >
        ...
        <div className="flex flex-col col-span-2 gap-2">
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select
                disabled={new Date(task?.ended_at ? task.ended_at : "").getTime() < new Date().getTime()}
                onValueChange={field.onChange}
                value={field.value}
                defaultValue={field.value}
              >
